...
class Frame:
    ...
    depth_m: np.ndarray    # HxW float32 metres (0 = no reading)
...
class CamClient:
    def __init__(self, base: str = "http://127.0.0.1:8765", timeout: float = 1.0):
        ...
        self.scale = float(self.info["depth_scale"])
    ...
    def frame(self) -> Frame | None:
        ...
        depth = cv2.imdecode(np.frombuffer(png, np.uint8), cv2.IMREAD_UNCHANGED).astype(np.float32) * self.scale
...
if __name__ == "__main__":
    ...
    print(c.info)
    f = c.frame()
    print("frame", f.n, f.color.shape, "depth centre m", float(f.depth_m[f.depth_m.shape[0] // 2, f.depth_m.shape[1] // 2]))
