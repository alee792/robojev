...
def build(cfg: Config, w: World, brain=None) -> dict:
    ...
    q["target"] = {
        ...
        "instructions": {
            ...
            "rules": "Objects are seen by a depth camera and described only by colour, size and shape, so e.g. a paper cup appears as a 'white upright object' and a small cardboard box as an 'orange upright object'. Colour names come from a camera and can be off by a shade (black vs dark gray, white vs light gray, orange vs tan). Use colour and size to tell similar shapes apart: a paper cup is white or light coloured; a cardboard box is brown, orange or tan. [OVERFIT: this sentence was added to make one sim scene work; see docs/assessment.md] Pick the object that most plausibly matches the request. If the arm is already holding an object, that object is the target. If the request is a standing rule about where things should be (e.g. keep X on the mat and Y off it), pick the object that currently breaks the rule (see each object's `resting_on`); a flat mat is never the target. If nothing breaks the rule, choose none_of_these. Choose none_of_these only when there is no request or nothing plausibly matches.",
        },
